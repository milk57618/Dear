...
if (mysqli_connect_errno ( $conn = mysqli_connect ( "localhost", "root", "root", "pos" ) )) {
	...
	if ($id == "") {
		...
		$ret = mysqli_query ( $conn, "select * from customer where UserId = '$id'" );
		$ret2 = mysqli_query ( $conn, "select * from seller where UserId = '$id'" );
		if ( mysqli_fetch_array ( $ret ) || mysqli_fetch_array ( $ret2 ) ) {
			echo "<script>alert('동일한 아이디를 가진 회원이 존재합니다.')</script>";
		}
		else {
			mysqli_query ( $conn, "insert into customer(UserId, UserPw, Money) values('$id', HEX(AES_ENCRYPT('$pw', 'secret')), 0)" );
			mysqli_close ( $conn );
